...
class TimeSeriesDataset_SinglePandasDF_Inference(Dataset):
    """
    Inference-only dataset for a SINGLE pandas DataFrame (no CSV I/O).

    Output signature:
        (x_context, x_padding, freq, x_future)    # same as CSV version
        (+ optional meta dict if return_meta=True)

    Shapes/dtypes:
        x_context: [L, 1], float32
        x_padding: [L, 1], float32
        freq:      [1],   int64
        x_future:  [0, 1], float32

    Modes:
      - sliding_windows=False: return only the last window for each column.
      - sliding_windows=True:  stride=1 windows across the series.
    """
    ...
    def __init__(
        ...
        df_name: str = "<dataframe>",   # used only for metadata
    ):
        super().__init__()
        if context_length <= 0:
            raise ValueError("context_length must be positive.")
        ...
        if columns is None:
            ...
            for c in columns:
                if isinstance(c, int):
                    if c < 0 or c >= ncols:
                        raise IndexError(
                            f"Column index {c} out of range [0, {ncols-1}] for DataFrame."
                        )
                    use_cols.append(df.columns[c])
                elif isinstance(c, str):
                    if c not in df.columns:
        ...
        self.series_col_indices: List[int] = []
        ...
        for name in use_cols:
            ...
            if self.dropna:
                ...
                if np.isnan(arr).any():
                    raise ValueError(
                        f"Column '{name}' contains NaNs; set dropna=True or clean first."
                    )
        ...
        # ---- Sliding window index build (same as CSV)
        self.index_records: List[Tuple[int, int]] = []
        if self.sliding_windows:
            for sidx, arr in enumerate(self.series_arrays):
                n = len(arr)
                for start in range(0, n - self.L + 1):
    ...
    def _make_meta(self, series_idx: int, window_start: int) -> Dict[str, Any]:
        return {
            "csv_path": self.df_name,          # for compatibility
            "series_idx": int(series_idx),
            "series_name": self.series_names[series_idx],
            "column_idx": self.series_col_indices[series_idx],
            "context_length": int(self.L),
    ...
    def __getitem__(self, idx: int):
        ...
        x_context = torch.as_tensor(ctx_np, dtype=torch.float32).unsqueeze(-1)
        x_padding = torch.zeros((self.L, 1), dtype=torch.float32)
        freq = torch.tensor([self.freq_type], dtype=torch.long)
        x_future = torch.empty((0, 1), dtype=torch.float32)
